...
int main(int argc, char *argv[])
{
	...
	if (unlink(NOMESOCK) == -1)
	{						   // unlink() serve prima di bind()
	...
	while (!terminated && 
		  (connect_socket = accept(server_socket, (struct sockaddr *)&client_addr, &client_addr_len)) != -1)
	{
		// cycle: process data from accepted connection
		printf("Server: new connection from client\n");
		while ((retcode = read(connect_socket, buffer, MAXBUF)) > 0) // multiple read()s likely if MAXBUF small
...
void terminazione(int signo)
{
	...
	printf(MOREMSG);
	return; // go to main's blocking accept()
